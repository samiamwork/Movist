                  ...
                  error:(NSError**)error
{
	...
	NSString* fps = (NSString*)[options objectForKey:@"movieFps"];
	TRACE(@"Movie FPS: %f", [fps floatValue]);
    NSRange r;
    NSString* s;
	...
	if([_source matchedByPattern:@"^{"]) {
		...
		TRACE(@"MicroDVD subtitles format");
	} else if ([_source matchedByPattern:@"^\\["]) {
		[subtitle setName:@"MPL2"];
		TRACE(@"MPL2 subtitles format");
	} else if ([_source matchedByPattern:@"^[0-9]"]) {
		[subtitle setName:@"TMPlayer"];
		TRACE(@"TMPlayer subtitles format");
	}
    while (0 < _sourceRange.length) {
        r = [_source tokenRangeForDelimiterSet:set rangePtr:&_sourceRange];
        if (r.length != 0) {
            s = [_source substringWithRange:r];
            if ([subtitle name] == @"MicroDVD") {
				sub = [s substringsCapturedByPattern:@"{([0-9]+)}{([0-9]+)}(.*)"];
				beginTime = (float)([[sub objectAtIndex:1] floatValue] / [fps floatValue]);
				endTime = (float)([[sub objectAtIndex:2] floatValue] / [fps floatValue]);
				ms = [NSMutableString stringWithString:[sub objectAtIndex:3]];
			} else if ([subtitle name] == @"MPL2") {
				sub = [s substringsCapturedByPattern:@"\\[([0-9]+)\\]\\[([0-9]+)\\](.*)"];
				beginTime = (float)([[sub objectAtIndex:1] floatValue] / 10.0);
				TRACE(@"%f",beginTime);
				endTime = (float)([[sub objectAtIndex:2] floatValue] / 10.0);
				ms = [NSMutableString stringWithString:[sub objectAtIndex:3]];
			} else if ([subtitle name] == @"TMPlayer") {
				sub = [s substringsCapturedByPattern:@"([0-9]+)\\:([0-9]+)\\:([0-9]+).(.*)"];
				hour = [[sub objectAtIndex:1] intValue];
				min = [[sub objectAtIndex:2] intValue];
				sec = [[sub objectAtIndex:3] intValue];
				ms = [NSMutableString stringWithString:[sub objectAtIndex:4]];
				beginTime = (hour * 60 * 60) + (min * 60) + (sec);
				endTime = -1;
			}
