...
int
vobsub_get_next_packet(void *vobhandle, int index, void** data, int* timestamp)
{
  vobsub_t *vob = (vobsub_t *)vobhandle;
  if (vob->spu_streams && 0 <= index && (unsigned)index < vob->spu_streams_size) {
    packet_queue_t *queue = vob->spu_streams + index;
    if (queue->current_index < queue->packets_size) {
